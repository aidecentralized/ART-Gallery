...
export const testApiConnection = async (useDirect = false) => {
  try {
    ...
    console.log(`Testing API connection to ${useDirect ? 'direct NANDA backend' : 'proxy API'}...`);
    const baseUrl = useDirect ? 'http://nanda.us-east-2.elasticbeanstalk.com/api/v1' : '/api/v1';
    ...
    const serverResponse = await client.get(`${useDirect ? baseUrl : baseUrl}/servers/`);
    ...
    return {
      success: true,
      message: `API connection successful to ${useDirect ? 'direct backend' : 'proxy'}`,
      baseUrl
    };
  } catch (error) {
...
export const testRegistration = async (userData, useDirect = false) => {
  try {
    ...
    console.log(`Testing registration with data via ${useDirect ? 'direct API' : 'proxy'}:`, userData);
    
    const response = await client.post(`${useDirect ? baseUrl : baseUrl}/auth/register/`, userData);
...
export const testLogin = async (email, password, useDirect = false) => {
  try {
    ...
    console.log(`Testing login for ${email} via ${useDirect ? 'direct API' : 'proxy'}`);
    
    const response = await client.post(`${useDirect ? baseUrl : baseUrl}/auth/token/`, { email, password });
